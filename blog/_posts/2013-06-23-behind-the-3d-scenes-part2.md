...
We start where we left of in the [Loading a scene with libGDX]({% post_url 2013-05-29-loading-a-scene-with-libgdx %}) tutorial. We will break down the code to get a glimpse behind the scenes. Therefor you might want to make a backup or a copy to work on. For reference here is the code:
...
We create a ModelLoader, which we have seen earlier in [Loading models using libGDX]({% post_url 2013-05-25-loading-models-using-libgdx %}). But instead of the ObjLoader we used earlier we now create a G3dModelLoader. We construct it with a JsonReader as argument, because the file format of the invaderscene.g3dj is json. For a g3db file you can use UBJsonReader.
...
<a href="behindthescenes1.png"><img src="behindthescenes1.png" alt="behindthescenes1" width="300" /></a>
...
<a href="behindthescenes2.png"><img src="behindthescenes2.png" alt="behindthescenes2" width="300" /></a>
...
Notice that ModelInstance implements the RenderableProvider interface. When we call `modelBatch.render(instance, lights);`, the ModelBatch actually expects a RenderableProvider instead of a ModelInstance. A class that implements a RenderableProvider supplies one or more Renderable objects to the ModelBatch. Take a look at the <a href="https://github.com/libgdx/libgdx/blob/master/gdx/src/com/badlogic/gdx/graphics/g3d/Renderable.java" title="Renderable.java" target="_blank">Renderable class</a>:
...
<a href="behindthescenes3.png"><img src="behindthescenes3.png" alt="behindthescenes3" width="300" /></a>
...
[Next: Creating a shader with libGDX]({% post_url 2013-07-19-creating-a-shader-with-libgdx %})
